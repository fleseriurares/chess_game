...
namespace ChessModel
{
    public class Logic
    {
        ...
        public void AddIfEnPassant(Pawn pawn)
        {
            ...
            {
                if (pos.X < 7 && Board[pos.X + 1, pos.Y] != null && Board[pos.X + 1, pos.Y ].PieceType == Type.Pawn && Board[pos.X + 1, pos.Y ].Side != pawn.Side)
                {
                    ((Pawn)Board[pos.X + 1, pos.Y]).enPassantCondDir[1] = true;
                    enPassant.Add(new Tuple<Pawn,Pawn>(pawn, (Pawn)Board[pos. X + 1, pos.Y]));
                }
                if (pos.X > 0 && Board[pos.X -1 , pos.Y] != null && Board[pos.X - 1, pos.Y ].PieceType == Type.Pawn && Board[pos.X - 1, pos.Y ].Side != pawn.Side)
                {
